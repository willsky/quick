...
defined('ERROR_MYSQL_CONNECT_FAIL') || define('ERROR_MYSQL_CONNECT_FAIL', 1000);
...
class Medoo {
    ...
    public function __construct($options = null) {
        try {
            ...
            if (
            in_array($type, explode(' ', 'mariadb mysql pgsql sybase mssql')) && $this->charset) {
            ...
            throw new\Exception($e->getMessage(), ERROR_MYSQL_CONNECT_FAIL);
        }
    }
    ...
    protected function data_implode($data, $conjunctor, $outer_conjunctor = null) {
        ...
        foreach($data as $key => $value) {
            ...
            if (
            preg_match("/^(AND|OR)(\s+#.*)?$/i", $key, $relation_match) && $type == 'array') {
    ...
    protected function where_clause($where) {
        ...
        if (is_array($where)) {
            ...
            if (isset($where['ORDER'])) {
                ...
                if (is_array($ORDER)) {
                    if (
                    isset($ORDER[1]) && is_array($ORDER[1])) {
